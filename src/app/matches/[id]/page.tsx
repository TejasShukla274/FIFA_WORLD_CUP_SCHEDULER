...
import { Star, MapPin, Calendar, Clock, Trophy, RefreshCw, CircleAlert, Sparkles } from 'lucide-react';
...
export default function MatchDetailsPage({ params }: MatchDetailsPageProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className={`glass-card overflow-hidden p-6 sm:p-10 border ${
        isFav ? 'border-brand-gold/40 shadow-gold-glow bg-brand-gold/5' : 'border-border-card'
      }`}>
        <div className="flex flex-col items-center text-center space-y-4">
          <span className="text-[11px] font-extrabold uppercase bg-white/5 border border-white/5 text-brand-gold px-3 py-1 rounded-full">
          ...
          <div className="w-full flex items-center justify-center gap-4 sm:gap-12 py-4">
            ...
            <div className="flex flex-col items-center flex-1 space-y-2">
              ...
              <span className="text-lg sm:text-2xl font-black text-white truncate max-w-[150px]">{t1Name}</span>
              <span className="text-xs text-text-muted font-bold tracking-widest uppercase">{t1Code}</span>
              {team1Obj && (
                <button
                  onClick={() => toggleFavorite(team1Obj.id)}
                  className={`p-1 rounded-full hover:bg-white/5 transition-all text-xs font-semibold flex items-center gap-1 cursor-pointer ${
            ...
            <div className="flex flex-col items-center justify-center min-w-[100px] gap-2">
              {match.is_completed ? (
                <div className="text-4xl sm:text-5xl font-black text-white bg-white/5 border border-white/5 px-4 sm:px-6 py-2 rounded-2xl shadow-inner">
                  {match.team1_score} - {match.team2_score}
                </div>
              ) : (
                <span className="text-xs font-black uppercase text-brand-gold tracking-widest px-3 py-1 bg-white/5 rounded-lg border border-white/5 shadow-inner">
                  VS
                </span>
              )}
              {match.is_completed && (
            ...
            <div className="flex flex-col items-center flex-1 space-y-2">
              ...
              <span className="text-lg sm:text-2xl font-black text-white truncate max-w-[150px]">{t2Name}</span>
          ...
          {/* Date, Time and Venue */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg w-full pt-6 border-t border-white/5 text-sm text-text-muted">
            <div className="flex items-center justify-center sm:justify-end gap-2">
              <Calendar className="text-brand-gold/60 animate-pulse" size={16} />
        ...
        <div className="space-y-6">
          ...
          {!match.is_completed && (
            <div className="p-5 rounded-2xl glass-card border border-border-card space-y-3">
              ...
              <div className="flex justify-center py-4 bg-black/10 rounded-xl">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full text-center py-3 bg-black/20 border border-border-card text-white font-black text-xl rounded-xl focus:outline-none focus:border-brand-gold"
                  />
                </div>
                <div className="space-y-1">
              ...
              <div className="flex items-center gap-3">
                ...
                {(match.is_completed || team1Score !== '' || team2Score !== '') && (
                  <button
                    ...
                    disabled={updating}
                    className="px-4 py-2.5 rounded-xl bg-white/5 border border-white/5 text-text-muted hover:text-white text-xs font-black uppercase tracking-wider transition-colors cursor-pointer interactive-scale"
                  >
                    Reset
                  </button>
                )}
            ...
            <div className="p-5 rounded-2xl glass-card border border-border-card space-y-3">
              ...
              <div className="space-y-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h4 className="text-sm font-extrabold text-white leading-tight">{venueObj.name}</h4>
                    <p className="text-[11px] text-text-muted">{venueObj.city}, {venueObj.country}</p>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/5 text-xs text-text-muted">
                  <div>
                    <span className="text-text-dark font-bold text-[10px] uppercase block">Stadium Capacity</span>
                    <span className="font-extrabold text-white">{venueObj.capacity} seats</span>
                  </div>
                  <div>
                    <span className="text-text-dark font-bold text-[10px] uppercase block">Host Country</span>
                    <span className="font-extrabold text-white">{venueObj.country}</span>
                  </div>
                </div>
              </div>
            </div>
